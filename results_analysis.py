...
docker = Docker(0.5, True)
doker_0 = Docker(0, True)
# Read source lines
    ...
    if smiles not in m_set:
        ...
        if v is not None:
            ...
            if centers[ec] is None:
                centers[ec] = doker_0.dock_src_ec(src, ec)
        ec_counter[ec] += 1
...
for single_ec in ec_to_filter:
    ...
    plt.scatter([x[0] for x in vec_in_ec], [x[1] for x in vec_in_ec], alpha=0.7, c=[ec_to_color(single_ec)],
                label=single_ec)
