...
if [ $SLURM_ARRAY_TASK_ID -le ${#RUN_NAMES[@]} ]; then
    ...
    python eval_gen.py --run_name $RUN_NAME --fast 0 --split test --dups 0 --per_level 1 --per_ds 1 --res_base ${RESULTS_DIR}
    python eval_gen.py --run_name $RUN_NAME --fast 0 --split test --dups 1 --per_level 1 --per_ds 1 --res_base ${RESULTS_DIR}
else
